...
namespace BlackCat\Database\Packages\RbacRolePermissions\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\RbacRolePermissions\Dto\RbacRolePermissionDto;
use BlackCat\Database\Packages\RbacRolePermissions\Mapper\RbacRolePermissionDtoMapper;
 ...
 */
final class RbacRolePermissionsAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private RbacRolePermissionRepository $rbacRolePermissionRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'rbac_role_permissions' => $this->rbacRolePermissionRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\RbacRolePermissions\ModuleException("Record {$idStr} not found for locking.");
            }
