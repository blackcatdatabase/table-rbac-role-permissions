...
use BlackCat\Database\Packages\RbacRolePermissions\Repository\RbacRolePermissionRepositoryInterface;
...
class RbacRolePermissionRepository implements RbacRolePermissionRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'role_id', 'permission_id' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\RbacRolePermissions\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'effect', 'source' ]);
          return count($rows);
        }
    ...
    public function getByRoleIdAndPermissionId(int $roleId, int $permissionId, bool $asDto = false): array|\BlackCat\Database\Packages\RbacRolePermissions\Dto\RbacRolePermissionDto|null {
        return $this->getByUnique([ 'role_id' => $roleId, 'permission_id' => $permissionId ], $asDto);
    }
    ...
    public function getIdByRoleIdAndPermissionId(int $roleId, int $permissionId) {
        $row = $this->getByRoleIdAndPermissionId($roleId, $permissionId, false);
        if (!is_array($row)) { return null; }
        return $row['role_id permission_id'] ?? null;
    }
