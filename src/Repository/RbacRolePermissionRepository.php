...
namespace BlackCat\Database\Packages\RbacRolePermissions\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\RbacRolePermissions\Definitions;
use BlackCat\Database\Packages\RbacRolePermissions\Criteria;
use BlackCat\Database\Packages\RbacRolePermissions\Dto\RbacRolePermissionDto as Dto;
use BlackCat\Database\Packages\RbacRolePermissions\Mapper\RbacRolePermissionDtoMapper as RowMapper;
...
final class RbacRolePermissionRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\RbacRolePermissions\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'role_id', 'permission_id' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\RbacRolePermissions\Definitions::table());
            return $bulk->upsertMany($rows, [ 'role_id', 'permission_id' ], [ 'effect', 'source' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\RbacRolePermissions\Dto\RbacRolePermissionDto|null */
    public function getByRoleIdAndPermissionId(int $roleId, int $permissionId, bool $asDto = false): array|\BlackCat\Database\Packages\RbacRolePermissions\Dto\RbacRolePermissionDto|null {
        $row = $this->getByUnique([ 'role_id' => $roleId, 'permission_id' => $permissionId ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\RbacRolePermissions\Mapper\RbacRolePermissionDtoMapper::fromRow($row);
    }
    public function existsByRoleIdAndPermissionId(int $roleId, int $permissionId): bool {
        $where = 't.' . Ident::q($this->db, 'role_id') . ' = :uniq_role_id' . ' AND ' . 't.' . Ident::q($this->db, 'permission_id') . ' = :uniq_permission_id';
        return $this->exists($where, [ 'uniq_role_id' => $roleId, 'uniq_permission_id' => $permissionId ]);
    }
    /** @return int|string|null */
    public function getIdByRoleIdAndPermissionId(int $roleId, int $permissionId) {
        $row = $this->getByRoleIdAndPermissionId($roleId, $permissionId);
        return $row ? ($row['role_id permission_id'] ?? null) : null;
    }
